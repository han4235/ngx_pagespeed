...
URL="http://$SECONDARY_HOSTNAME/mod_pagespeed_example/"
HEADERS="--header=Host:headers.example.com"
OUT=$($WGET_DUMP -O /dev/null -S $HEADERS $URL 2>&1)
...
URL="http://$SECONDARY_HOSTNAME/mod_pagespeed_example/combine_javascript2.js"
OUT=$($WGET_DUMP -O /dev/null -S $HEADERS $URL 2>&1)
MATCHES=$(echo "$OUT" | grep -c "Server: nginx/") || true
check [ $MATCHES -eq 1 ]

start_test Override server header in html flow.
URL="http://$SECONDARY_HOSTNAME/mod_pagespeed_test/whitespace.html"
OUT=$($WGET_DUMP -O /dev/null -S $HEADERS $URL 2>&1)
MATCHES=$(echo "$OUT" | grep -c "Server: override") || true
check [ $MATCHES -eq 1 ]

start_test Override server header in resource flow.
URL="http://$SECONDARY_HOSTNAME/mod_pagespeed_test/"
...
URL="http://$SECONDARY_HOSTNAME/mod_pagespeed_test/proxy_pass.css"
OUT=$($WGET_DUMP -O /dev/null -S $HEADERS $URL 2>&1)
MATCHES=$(echo "$OUT" | grep -c "Server: override") || true
check [ $MATCHES -eq 1 ]
